...
class quanto_contact_info_widget extends WP_Widget {
        function __construct() {
                parent::__construct(
                        ...
                        esc_html__( 'Agroland :: Cooperative Contact', 'agroland' ),
                        // Widget description
                        array(
                                'description'    => esc_html__( 'Add farm cooperative contact info', 'agroland' ),
...
public function widget( $args, $instance ) {
        ...
        $cooperative            = apply_filters( 'widget_cooperative_contact', $instance['cooperative_contact'] );
    ...
    echo '<!-- About Widget Start -->';
        if( !empty( $title ) || !empty( $address ) ||  !empty( $email ) || !empty( $mobile ) || !empty( $cooperative ) ):
                        ...
                        echo '</p>';
                                ...
                                if( ! empty( $cooperative ) ){
                                        echo '<p class="mb-0 link-inherit">';
                                                echo '<i class="fas fa-users me-3"></i>';
                                                echo esc_html( $cooperative );
                                        echo '</p>';
                                }
...
public function form( $instance ) {
        ...
        if ( isset( $instance[ 'title' ] ) ) {
                ...
                $title = esc_html__( 'Contact Our Cooperative', 'agroland' );
        }
        ...
        if ( isset( $instance[ 'address' ] ) ) {
                ...
                $address = esc_html__( '123 Green Way, Sustainable Valley', 'agroland' );
        }
    ...
    if ( isset( $instance[ 'cooperative_contact' ] ) ) {
        $cooperative_contact = $instance[ 'cooperative_contact' ];
    }else {
        $cooperative_contact = esc_html__( 'Agroland Farmers Cooperative', 'agroland' );
    }
?>
        <p>
        ...
        <p>
                <label for="<?php echo $this->get_field_id( 'cooperative_contact' ); ?>">
                        <?php
                                _e( 'Cooperative Contact:' ,'agroland');
                        ?>
                </label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'cooperative_contact' ); ?>" name="<?php echo $this->get_field_name( 'cooperative_contact' ); ?>" type="text" value="<?php echo esc_attr( $cooperative_contact ); ?>" />
        </p>
...
public function update( $new_instance, $old_instance ) {
        ...
        $instance['cooperative_contact']  = ( ! empty( $new_instance['cooperative_contact'] ) ) ? strip_tags( $new_instance['cooperative_contact'] ) : '';
